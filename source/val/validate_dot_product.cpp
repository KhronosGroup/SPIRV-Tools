...
spv_result_t ValidateSameSignedDot(ValidationState_t& _,
  ...
  const bool has_accumulator = opcode == spv::Op::OpSDotAccSat || opcode == spv::Op::OpUDotAccSat || opcode == spv::Op::OpSUDotAccSat;
  if (has_accumulator) {
  ...
  if (is_vec_1_scalar != is_vec_2_scalar) {
    ...
    if (result_width < 8) {
      return _.diag(SPV_ERROR_INVALID_DATA, inst)
            ...
            << ") must be greater than or equal to the packed vector width of 8";
    }

    // PackedVectorFormat4x8Bit is used when the "Vector" operand are really
    // scalar
    const uint32_t packed_operand = has_accumulator ? 6 : 5;
    const bool has_packed_vec_format = inst->operands().size() == packed_operand;
