...
class ValidationState_t {
 public:
  ...
  /// Sets the has a nested block/bufferblock decorated struct for a given struct ID
  void SetHasNestedBlockOrBufferBlockStruct(uint32_t id, bool has) {
    struct_has_nested_blockorbufferblock_struct_[id] = has;
  }

  /// For a given struct ID returns true if it has a nested block/bufferblock decorated struct
  bool HasNestedBlockOrBufferBlockStruct(uint32_t id) {
    bool debug =struct_has_nested_blockorbufferblock_struct_[id];
    return debug;
  }
 ...
 private:
  ...
  std::unordered_map<uint32_t, bool> struct_has_nested_blockorbufferblock_struct_;
