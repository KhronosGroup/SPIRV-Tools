...
FuzzerPassAddReadModifyWriteAtomicInstruction::
    FuzzerPassAddReadModifyWriteAtomicInstruction(
...
void FuzzerPassAddReadModifyWriteAtomicInstruction::Apply() {
  ForEachInstructionWithInstructionDescriptor(
      [this](opt::Function* function, opt::BasicBlock* block,
        ...
        // Randomly decide whether to try inserting atomic(read-modify-write)
        // instruction here.
        if (!GetFuzzerContext()->ChoosePercentage(
                GetFuzzerContext()
                    ->GetChanceOfAddingReadModifyWriteAtomicInstruction())) {
          return;
        }
        ...
        auto atomic_opcode =
            atomic_opcodes[GetFuzzerContext()->RandomIndex(atomic_opcodes)];

        // Check whether it is legitimate to insert atomic(read-modify-write)
        // instruction before this instruction.
        if (!fuzzerutil::CanInsertOpcodeBeforeInstruction(atomic_opcode,
                                                          inst_it)) {
          return;
        }
            ...
            FindAvailableInstructions(
                ...
                [](opt::IRContext* context,
                  ...
                  switch (instruction->opcode()) {
                    ...
                    case SpvOpUndef:
                      // Do not allow null or undefined pointer;
        ...
        uint32_t memory_semantics_id_1 = 0;
        uint32_t memory_semantics_id_2 = 0;
        uint32_t value_id = 0;
        uint32_t comparator_id = 0;
        ...
        switch (storage_class) {
          ...
          case SpvStorageClassImage:
            ...
            memory_semantics_id_1 = FindOrCreateConstant(
            ...
            switch (atomic_opcode) {
              ...
              case SpvOpAtomicFAddEXT:
                value_id = FindOrCreateConstant(
                    {/*value*/},
                    FindOrCreateIntegerType(32,
              ...
              case SpvOpAtomicCompareExchangeWeak:
                value_id = FindOrCreateConstant(
                    ...
                    FindOrCreateIntegerType(32,
                                            GetFuzzerContext()->ChooseEven()),
                    false);
                memory_semantics_id_2 = FindOrCreateConstant(
                    {static_cast<uint32_t>(
                        fuzzerutil::GetMemorySemanticsForStorageClass(
                            storage_class))},
                    FindOrCreateIntegerType(32,
                ...
                comparator_id = FindOrCreateConstant(
                    {/*value*/},
                    FindOrCreateIntegerType(32,
                                            GetFuzzerContext()->ChooseEven()),
                    false);
        ...
        ApplyTransformation(TransformationAddReadModifyWriteAtomicInstruction(
            GetFuzzerContext()->GetFreshId(), chosen_instruction->result_id(),
            static_cast<uint32_t>(atomic_opcode), memory_scope_id,
            memory_semantics_id_1, memory_semantics_id_2, value_id,
            comparator_id, instruction_descriptor));
      });
}
