...
bool CanInsertOpcodeBeforeInstruction(SpvOp opcode,
  ...
  if (opcode != SpvOpVariable && instruction_in_block->opcode() == SpvOpVariable) {
  ...
  // We cannot insert a non-OpPhi instruction directly before an OpPhi, because OpPhi instructions
...
bool CanMakeSynonymOf(opt::IRContext* ir_context,
  ...
  // We do not make synonyms of objects that have decorations: if the synonym is not
  // decorated analogously, using the original object vs. its synonymous form may not be
  // equivalent.
