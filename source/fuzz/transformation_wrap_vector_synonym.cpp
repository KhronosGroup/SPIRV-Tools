...
TransformationWrapVectorSynonym::TransformationWrapVectorSynonym(
    uint32_t instruction_id, uint32_t result_id1, uint32_t result_id2,
    uint32_t vec_id, uint32_t vec_type_id, uint32_t pos) {
...
bool TransformationWrapVectorSynonym::IsApplicable(
  ...
  auto vector_type =
      ir_context->get_type_mgr()->GetType(message_.vec_type_id());
  ...
  std::unordered_set<SpvOp_> valid_opcodes{SpvOpIAdd, SpvOpISub, SpvOpIMul,
  ...
  std::vector<uint32_t> vec_ids{message_.result_id1(), message_.result_id2(),
                                message_.vec_id()};
  ...
  if (vec1_type_id != message_.vec_type_id() ||
      vec2_type_id != message_.vec_type_id()) {
    return false;
  }
...
void TransformationWrapVectorSynonym::Apply(
  ...
  auto inst_descriptor = MakeInstructionDescriptor(message_.instruction_id(),
                                                   instruction->opcode(), 0);
  auto insert_before_inst = FindInstruction(inst_descriptor, ir_context);
