...
bool TransformationAddLocalVariable::IsApplicable(
  ...
  auto type_instruction = context->get_def_use_mgr()->GetDef(message_.type_id
          ());
  ...
  auto initializer_instruction = context->get_def_use_mgr()->GetDef(message_
          .initializer_id());
  // ... exist, ...
...
void TransformationAddLocalVariable::Apply(
  ...
  fuzzerutil::FindFunction(context, message_.function_id())->begin()->begin()
  ->InsertBefore(MakeUnique<opt::Instruction>(context,
          SpvOpVariable, message_.type_id(), message_.fresh_id(),
          opt::Instruction::OperandList({ { SPV_OPERAND_TYPE_STORAGE_CLASS, {

          SpvStorageClassFunction}},
                                          { SPV_OPERAND_TYPE_ID, { message_
                                          .initializer_id()}} })));
