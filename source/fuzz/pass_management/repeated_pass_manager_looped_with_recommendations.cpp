...
RepeatedPassManagerLoopedWithRecommendations::
    ...
    : RepeatedPassManager(fuzzer_context, pass_instances), num_transformations_applied_before_last_pass_choice_(0), next_pass_index_(0) {
  ...
  do {
    ...
    for (auto future_pass :
      ...
      recommended_pass_indices_.insert(static_cast<uint32_t>(pass_loop_.size()));
...
FuzzerPass* RepeatedPassManagerLoopedWithRecommendations::ChoosePass(const protobufs::TransformationSequence& applied_transformations) {
  assert ((next_pass_index_ > 0 || recommended_pass_indices_.count(next_pass_index_) == 0) && "The first pass in the loop should not be a recommendation.");
  ...
  if (num_transformations_applied_before_last_pass_choice_ == static_cast<uint32_t>(applied_transformations.transformation_size())) {
    // The last pass that was applied did not lead to any new transformations.  We thus do not want to apply recommendations based on it, so we skip on to the next non-recommended pass.
    while (recommended_pass_indices_.count(next_pass_index_)) {
      next_pass_index_ = (next_pass_index_ + 1) % static_cast<uint32_t>(pass_loop_.size());
    }
  }
