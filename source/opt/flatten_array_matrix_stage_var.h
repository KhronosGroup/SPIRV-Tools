...
class FlattenArrayMatrixStageVariable : public Pass {
 ...
 private:
  ...
  std::vector<Instruction*> GetUsersIf(
  ...
  FlattenedVariables CreateFlattenedStageVarsForReplacement(
  ...
  // flattened variables.
  FlattenedVariables CreateFlattenedStageVarsForArray(
      Instruction* stage_var_type, SpvStorageClass storage_class,
      uint32_t extra_array_length);
  ...
  // |extra_array_length| is not zero, adds the extra arrayness to all the
  // flattened variables.
  FlattenedVariables CreateFlattenedStageVarsForMatrix(
      Instruction* stage_var_type, SpvStorageClass storage_class,
      uint32_t extra_array_length);
