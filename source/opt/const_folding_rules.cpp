    ...
    return [](IRContext* context, Instruction* inst,
        ...
        if (float_type->width() == 32) {
            for (uint32_t i = 0; i < c2_components.size(); ++i) {
                ...
                for (uint32_t j = 0; j < c2_components[i]->AsVectorConstant()->GetComponents().size(); ++j) {
                    float c1_scalar = c1_components[j]->GetFloat();
                    float c2_scalar = c2_components[i]->AsVectorConstant()->GetComponents()[j]->GetFloat();
            ...
            for (uint32_t i = 0; i < c2_components.size(); ++i) {
                ...
                for (uint32_t j = 0; j < c2_components[i]->AsVectorConstant()->GetComponents().size(); ++j) {
                    ...
                    double c2_scalar = c2_components[i]->AsVectorConstant()->GetComponents()[j]->GetDouble();
                    result_scalar += c1_scalar * c2_scalar;
                }
...
ConstantFoldingRule FoldMatrixTimesVector() {
    return [](IRContext* context, Instruction* inst,
        ...
        std::vector<uint32_t> ids;
        if (float_type->width() == 32) {
            for (uint32_t i = 0; i < c1_components[0]->AsVectorConstant()->GetComponents().size(); ++i) {
                float result_scalar = 0.0f;
                for (uint32_t j = 0; j < c1_components.size(); ++j) {
                    float c1_scalar = c1_components[j]->AsVectorConstant()->GetComponents()[i]->GetFloat();
            ...
            for (uint32_t i = 0; i < c1_components[0]->AsVectorConstant()->GetComponents().size(); ++i) {
                double result_scalar = 0.0;
                for (uint32_t j = 0; j < c1_components.size(); ++j) {
                    double c1_scalar = c1_components[j]->AsVectorConstant()->GetComponents()[i]->GetDouble();
