...
spv_result_t AtomicsPass(ValidationState_t& _,
  ...
  switch (opcode) {
    ...
    case SpvOpAtomicXor: {
      ...
      const uint32_t pointer_type = _.GetOperandTypeId(inst, 2);
      ...
      const uint32_t scope_type = _.GetOperandTypeId(inst, 3);
      ...
      const uint32_t memory_semantics_type1 = _.GetOperandTypeId(inst, 4);
      ...
      if (opcode != SpvOpAtomicLoad && opcode != SpvOpAtomicIIncrement &&
      ...
      if (opcode == SpvOpAtomicCompareExchange ||
          opcode == SpvOpAtomicCompareExchangeWeak) {
        const uint32_t comparator_type = _.GetOperandTypeId(inst, operand_index++);
    ...
    case SpvOpAtomicStore: {
      const uint32_t pointer_type = _.GetOperandTypeId(inst, 0);
      uint32_t data_type = 0;
      uint32_t storage_class = 0;
      if (!_.GetPointerTypeInfo(pointer_type, &data_type, &storage_class)) {
        return _.diag(SPV_ERROR_INVALID_DATA)
      ...
      const uint32_t scope_type = _.GetOperandTypeId(inst, 1);
      if (!_.IsIntScalarType(scope_type) || _.GetBitWidth(scope_type) != 32) {
        return _.diag(SPV_ERROR_INVALID_DATA)
               << spvOpcodeString(opcode)
               << ": expected Scope to be 32-bit int";
      ...
      const uint32_t memory_semantics_type = _.GetOperandTypeId(inst, 2);
