...
OpControlBarrier %workgroup %device %acquire_release_subgroup
  ...
  ASSERT_EQ(SPV_ERROR_INVALID_DATA, ValidateInstructions(SPV_ENV_VULKAN_1_0));
  EXPECT_THAT(getDiagnosticString(),
              AnyVUID("VUID-StandaloneSpirv-MemorySemantics-10870"));
  EXPECT_THAT(
      getDiagnosticString(),
      HasSubstr(
...
OpControlBarrier %workgroup %workgroup %workgroup_memory
  ...
  ASSERT_EQ(SPV_ERROR_INVALID_DATA, ValidateInstructions(SPV_ENV_VULKAN_1_0));
  EXPECT_THAT(getDiagnosticString(),
              AnyVUID("VUID-StandaloneSpirv-MemorySemantics-10871"));
  EXPECT_THAT(
      getDiagnosticString(),
      HasSubstr(
          "ControlBarrier: Memory Semantics with at least one Vulkan-supported "
          "storage class semantics bit set (UniformMemory, WorkgroupMemory, "
...
OpMemoryBarrier %device %none
  ...
  ASSERT_EQ(SPV_ERROR_INVALID_DATA, ValidateInstructions(SPV_ENV_VULKAN_1_0));
  EXPECT_THAT(getDiagnosticString(),
              AnyVUID("VUID-StandaloneSpirv-MemorySemantics-10869"));
  EXPECT_THAT(getDiagnosticString(),
              HasSubstr("MemoryBarrier: MemorySemantics must not use Relaxed "
                        "memory order with MemoryBarrier"));
}
...
OpMemoryBarrier %device %acquire
  ...
  EXPECT_THAT(
      getDiagnosticString(),
      HasSubstr("MemoryBarrier: Memory Semantics with a non-relaxed memory "
                "order (Acquire, Release, or AcquireRelease) must have at "
                "least one Vulkan-supported storage class semantics bit set "
...
OpFunctionEnd
  ...
  EXPECT_THAT(
      getDiagnosticString(),
      HasSubstr("ControlBarrier: Memory Semantics with MakeAvailable bit set "
...
OpFunctionEnd
  ...
  EXPECT_THAT(
      getDiagnosticString(),
      HasSubstr("ControlBarrier: Memory Semantics with MakeVisible bit set "
...
OpFunctionEnd
  ...
  EXPECT_THAT(
      getDiagnosticString(),
      HasSubstr(
          "MemoryBarrier: Memory Semantics with a non-relaxed memory order "
          "(Acquire, Release, or AcquireRelease) must have at least one "
          "Vulkan-supported storage class semantics bit set (UniformMemory, "
          "WorkgroupMemory, ImageMemory, or OutputMemory)"));
}
...
OpFunctionEnd
  ...
  EXPECT_EQ(SPV_ERROR_INVALID_DATA,
            ValidateInstructions(SPV_ENV_UNIVERSAL_1_3));
  EXPECT_THAT(getDiagnosticString(),
              HasSubstr("Memory Semantics with Volatile bit set must not be "
                        "used with barrier instructions"));
}
