...
TEST_F(ValidateBuiltIns, VulkanCullPrimitiveEXTSuccess) {
  const std::string declarations = R"(
%array = OpTypeArray %bool %uint_16
%array_ptr = OpTypePointer Output %array
%var = OpVariable %array_ptr Output
...
%access = OpAccessChain %ptr %var %int_0
  ...
  CompileSuccessfully(
      GenerateMeshShadingCode("CullPrimitiveEXT", "OutputPoints", body,
                              declarations)
          .c_str(),
      SPV_ENV_VULKAN_1_2);
  EXPECT_EQ(SPV_SUCCESS, ValidateInstructions(SPV_ENV_VULKAN_1_2));
...
TEST_F(ValidateBuiltIns, VulkanCullPrimitiveEXTStorageClass) {
  const std::string declarations = R"(
%array = OpTypeArray %bool %uint_16
%array_ptr = OpTypePointer Input %array
%var = OpVariable %array_ptr Input
%ptr = OpTypePointer Input %bool
)";
  const std::string body = R"(
%access = OpAccessChain %ptr %var %int_0
)";
...
TEST_F(ValidateBuiltIns, VulkanCullPrimitiveEXTType) {
  const std::string declarations = R"(
%array = OpTypeArray %v3uint %uint_16
%array_ptr = OpTypePointer Output %array
%var = OpVariable %array_ptr Output
