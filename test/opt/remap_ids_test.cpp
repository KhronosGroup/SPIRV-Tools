...
using RemapIdsTest = PassTest<::testing::Test>;

// ported from remap.basic.everything.frag
TEST_F(RemapIdsTest, remap_basic) {
...
OpFunctionEnd
  ...
  SinglePassRunAndCheck<RemapIdsPass>(before, after, false, false);
}

// ported from remap.hlsl.sample.basic.everything.frag
TEST_F(RemapIdsTest, remap_hlsl_sample_basic) {
  const std::string before =
      R"(OpCapability Shader
...
TEST_F(RemapIdsTest, remap_hlsl_templatetypes) {
  const std::string before =
      R"(OpCapability Shader
...
TEST_F(RemapIdsTest, remap_similar_1a) {
  const std::string before =
      R"(OpCapability Shader
%1 = OpExtInstImport "GLSL.std.450"
OpMemoryModel Logical GLSL450
...
TEST_F(RemapIdsTest, remap_similar_1b) {
  const std::string before =
      R"(OpCapability Shader
%1 = OpExtInstImport "GLSL.std.450"
OpMemoryModel Logical GLSL450
...
TEST_F(RemapIdsTest, remap_specconst) {
  const std::string before =
      R"(OpCapability Shader
%1 = OpExtInstImport "GLSL.std.450"
OpMemoryModel Logical GLSL450
...
TEST_F(RemapIdsTest, remap_switch) {
  const std::string before =
      R"(OpCapability Shader
%1 = OpExtInstImport "GLSL.std.450"
OpMemoryModel Logical GLSL450
...
TEST_F(RemapIdsTest, remap_uniformarray) {
  const std::string before =
      R"(OpCapability Shader
%1 = OpExtInstImport "GLSL.std.450"
OpMemoryModel Logical GLSL450
