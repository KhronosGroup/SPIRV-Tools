...
//
//  TransformationCopyObject copy_true(100, 7, 5, 1);
...
//
//  TransformationCopyObject copy_false(101, 8, 100, 1);
...
//
//  TransformationCopyObject copy_false_again(102, 101, 5, 3);
...
//
//  TransformationCopyObject copy_true_again(103, 7, 102, 1);
...
//  ASSERT_FALSE(TransformationCopyObject(200, 18, 21, 0).IsApplicable(context.get(), fact_manager));
//
//  // Inapplicable because %77 is decorated.
//  ASSERT_FALSE(TransformationCopyObject(200, 17, 17, 1).IsApplicable(context.get(), fact_manager));
//
//  // Inapplicable because %80 is decorated.
//  ASSERT_FALSE(TransformationCopyObject(200, 80, 77, 0).IsApplicable(context.get(), fact_manager));
//
//  // Inapplicable because %84 is not available at the requested point
//  ASSERT_FALSE(TransformationCopyObject(200, 84, 32, 1).IsApplicable(context.get(), fact_manager));
//
//  // Fine because %84 is available at the requested point
//  ASSERT_TRUE(TransformationCopyObject(200, 84, 32, 2).IsApplicable(context.get(), fact_manager));
//
//  // Inapplicable because id %9 is already in use
//  ASSERT_TRUE(TransformationCopyObject(9, 84, 32, 2).IsApplicable(context.get(), fact_manager));
//
//  // Inapplicable because the requested point is not in a block
//  ASSERT_FALSE(TransformationCopyObject(200, 84, 86, 3).IsApplicable(context.get(), fact_manager));
//
//  // Inapplicable because %9 is not in a function
//  ASSERT_FALSE(TransformationCopyObject(200, 9, 9, 1).IsApplicable(context.get(), fact_manager));
//
//  // Inapplicable because %9 is not in a function
//  ASSERT_FALSE(TransformationCopyObject(200, 9, 9, 1).IsApplicable(context.get(), fact_manager));
//
//  // Inapplicable because the insert point is right before, or inside, a chunk of OpPhis
//  ASSERT_FALSE(TransformationCopyObject(200, 9, 30, 1).IsApplicable(context.get(), fact_manager));
//  ASSERT_FALSE(TransformationCopyObject(200, 9, 99, 1).IsApplicable(context.get(), fact_manager));
//
//  // OK, because the insert point is just after a chunk of OpPhis.
//  ASSERT_TRUE(TransformationCopyObject(200, 9, 96, 1).IsApplicable(context.get(), fact_manager));
//
//  // Inapplicable because the insert point is right after an OpSelectionMerge
//  ASSERT_FALSE(TransformationCopyObject(200, 9, 58, 2).IsApplicable(context.get(), fact_manager));
//
//  // OK, because the insert point is right before the OpSelectionMerge
//  ASSERT_TRUE(TransformationCopyObject(200, 9, 58, 1).IsApplicable(context.get(), fact_manager));
//
//  // Inapplicable because the insert point is right after an OpSelectionMerge
//  ASSERT_FALSE(TransformationCopyObject(200, 9, 43, 2).IsApplicable(context.get(), fact_manager));
//
//  // OK, because the insert point is right before the OpSelectionMerge
//  ASSERT_TRUE(TransformationCopyObject(200, 9, 43, 1).IsApplicable(context.get(), fact_manager));
//
//  // Inapplicable because the insert point is right after an OpLoopMerge
//  ASSERT_FALSE(TransformationCopyObject(200, 9, 40, 2).IsApplicable(context.get(), fact_manager));
//
//  // OK, because the insert point is right before the OpLoopMerge
//  ASSERT_TRUE(TransformationCopyObject(200, 9, 40, 1).IsApplicable(context.get(), fact_manager));
//
//  // Inapplicable because id %300 does not exist
//  ASSERT_TRUE(TransformationCopyObject(200, 300, 40, 1).IsApplicable(context.get(), fact_manager));
//
//  // TODO: check we don't copy a variable in a contiguous group of variables.
//  FAIL();
//
//}
//
//// TODO: make copy of OpVariable in function
//// TODO: make copy of OpVariable at global scope
//// TODO: make copy of uniform OpVariable
//// TODO: make copy of result of access chain
