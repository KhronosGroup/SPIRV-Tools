...
TEST(FuzzerUtilMaybeGetStructTypeTest, BasicTest) {
  std::string shader = R"(
               OpCapability Shader
  ...
  //  |scalar_type_id| must be a result id of a scalar type (i.e. int, float or
  //  bool).
  // Returns 0 if no such
  // instruction is present in the module. The returned id either participates
  // in IdIsIrrelevant fact or not, depending on the |is_irrelevant| parameter.
}
