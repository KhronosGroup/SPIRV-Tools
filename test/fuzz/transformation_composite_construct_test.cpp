...
TEST(TransformationCompositeConstructTest, HandlesIrrelevantIds) {
  std::string shader = R"(
               OpCapability Shader
          %1 = OpExtInstImport "GLSL.std.450"
               OpMemoryModel Logical GLSL450
  ...
  // Not applicable if one of components is irrelevant.
  fact_manager.AddFactIdIsIrrelevant(28);
  ASSERT_FALSE(
      TransformationCompositeConstruct(
          32, {25, 28, 31}, MakeInstructionDescriptor(31, SpvOpReturn, 0), 200)
          .IsApplicable(context.get(), transformation_context));

  // Not applicable if two of components is irrelevant.
