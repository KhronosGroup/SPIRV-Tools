...
TEST(FuzzerUtilMaybeGetVectorTypeTest, BasicTest) {
  std::string shader = R"(
               OpCapability Shader
  ...
  ASSERT_TRUE(fuzzerutil::MaybeGetVectorType(ir_context, 7, 4) == 90);

  // The vector type with |element_count| 3 and |component_type_id| 7
  // is not present in the module.
  ASSERT_TRUE(fuzzerutil::MaybeGetVectorType(ir_context, 7, 3) == 0);
